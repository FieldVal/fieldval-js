    ...
    get: function(field_name, desired_type, required) {
        ...
        if (arguments.length > 3) {
            ...
            for (var i = 3; i < arguments.length; i++) {
                ...
                var check = this_operator[0](value, function(new_value){
                    value = new_value;
                });
                if (check != null) {
                    fv.invalid(field_name, check);
                    had_error = true;
                    if(this_operator[1]==null || this_operator[1]){//stop_if_error flag (defaults to true)
                        break;
                    }
                }
            }
...
Validator.ChainVal = require('./ChainVal');
